...
class ImageStorage(FileSystemStorage):
    ...
    def _save(self, name, content):
        ...
        # 定义文件名，年月日时分秒随机数
        image_name = self.IMG_PREFIX + self.FILE_TIME + '_%s' + '%s' % (random.randint(0, 100), ext)
        # 重写合成文件名
        name = os.path.join(image_path, image_name)
        # 调用父类方法
...
class User(models.Model):
    ...
    icon = models.ImageField(verbose_name=u'头像', max_length=100, upload_to='upload/img/%Y%m%d',
                             default=u"apps/static/img/default.png", storage=ImageStorage())
...
class Review(models.Model):
    ...
    shop = models.ForeignKey('shop.Shop', models.DO_NOTHING, db_column='shop_id', db_index=True, verbose_name="商品ID")
    user = models.ForeignKey('account.User', models.DO_NOTHING, db_column='uid', db_index=True, verbose_name='用户ID')
...
class Order(models.Model):
    ...
    user = models.ForeignKey('account.User', models.DO_NOTHING, db_column='uid', verbose_name="属性ID")
...
class ShopCar(models.Model):
    car_id = models.IntegerField(verbose_name='ID', primary_key=True)
    number = models.IntegerField(verbose_name='商品数量', default=0)
    shop = models.ForeignKey(Shop, models.DO_NOTHING, verbose_name='商品ID')
    user = models.ForeignKey('account.User', models.DO_NOTHING, db_column='uid', verbose_name='用户ID')
...
class SubMenu2(models.Model):
    ...
    sub_menu = models.ForeignKey(SubMenu, models.DO_NOTHING, db_column='sub_menu_id', db_index=True, verbose_name='父菜单')
